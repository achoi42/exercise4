...
public class QuoteControllerMonthlyAggregateTest {
  ...
  @Test
  public void testMonthlyAggregate() throws Exception{
    ...
    when(quoteService.monthlyAggregate(anyString(), anyString()))
        .thenReturn(Arrays.asList(maxAg, minAg, totalAg));

    mockMvc
        .perform(get("/monthly/AAPL/2018-01"))
  ...
  @Test
  public void testMonthlyAggregateSymbolNotFound() throws Exception {
    when(quoteService.monthlyAggregate(anyString(), anyString()))
        .thenThrow(new BadRequestException());

    mockMvc
        .perform(get("/monthly/BAD/2018-06"))
        .andExpect(status().is4xxClientError());
  }

  @Test
  public void testMonthlyAggregateNoData() throws Exception {
    ...
    when(quoteService.monthlyAggregate(anyString(), anyString()))
        .thenReturn(Arrays.asList(max, min, total));

    mockMvc
        .perform(get("/monthly/AAPL/1234-99-99"))
        .andExpect(jsonPath("$[0].title", is(equalTo("highest price"))))
        .andExpect(jsonPath("$[1].title", is(equalTo("lowest price"))))
        .andExpect(jsonPath("$[2].title", is(equalTo("total volume"))))
